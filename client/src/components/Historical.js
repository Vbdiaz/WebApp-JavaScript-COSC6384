...
const socketHistorical = io("http://localhost:5000/historical");
...
function Historical() {
    ...
    useEffect(() => {
        socketHistorical.on("updateHistorical", (data) => {
            ...
            const convertedData = data.map(item => ({
                ...
                threshold_line2: item.portfolio_value * item.percent_threshold
            }));
        ...
        return () => {
            socketHistorical.disconnect();
        };
    }, []);

    return (
        <div>
            <h1>Historical Method VaR</h1>
            <div className="historical-container">
                <div className="historical-left-section">
                    <ResponsiveContainer width="100%" height={300}>
                        ...
                        >
                            ...
                            <Line type="monotone" dataKey="threshold_line2" stroke="#ff4d4f" strokeDasharray={"5 5"} dot={false}/>
                        </LineChart>
                    </ResponsiveContainer>
                </div>
                <div className="historical-right-section">
                    <table className="historical-table">
                        ...
                        <tbody>
                            {tableData.map((item, index) => (
                                  <tr
                                  key={index}
                                  className={item.warning === 1 ? 'historical-warning-row' : ''} // Apply the 'warning-row' class if warning is 1
                                >
